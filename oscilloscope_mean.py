...
def calculate_voltage_and_current_mean(pulse_percentage, path):

    files = find_csv_files_in_folder(path)

    voltage_mean = calculate_mean(pulse_percentage, files[voltage_channel - 1])
    current_mean = calculate_mean(pulse_percentage, files[current_channel - 1])
